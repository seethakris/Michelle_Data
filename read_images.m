...
%Input : Folder_Name - Experiment folder 
%Output : Folder_List - List of odor folders
%           T - time points
%           Z - stack
...
for ii = 1:length(Num_odors)    
    ...
    if ~strncmp(Num_odors(ii).name,'.',1)
        if isdir([Folder_Name,Num_odors(ii).name])
            ...
            %If there are .stk files present, then convert to tiff else
            %skip
            if ~isempty(Num_files)
                Folder_List{count} = char(Num_odors(ii).name);
